      ...
      if (!empty($_POST)) {
        ...
        $sql = 'INSERT INTO `questions` SET `member_id`=?,
                                           ...
                                           `article_id`=?,
                                            `answer_id`=?,
                                              `created`=NOW()';
        $data = array($_GET['member_id'], $_POST['content'], $_SESSION['article_id'], $_SESSION['login_member_id']);
        $stmt = $dbh->prepare($sql);
        $stmt->execute($data);
            ...
            <div class="row">
              ...
              <div class="col-xs-12 col-sm-4 col-md-5">
                <div>
                   ...
                   <?php foreach ($chatroom as $record) {
                    $id = $record['member_id'];
                    $name = $record['name']; ?>
                   <button type="submit" class=" btn btn-primary col-xs-12" onClick="location.href='answer.php?article_id=<?php echo $_SESSION['article_id']; ?>&member_id=<?php echo $id; ?>'"><?php echo $name; ?>さんからの質問</button><br><br>
                  <?php } } ?>
                </div>
              </div>
                  ...
                  <form method="post" action="" class="form-horizontal" role="form">
                    <div class="panel-footer">
                      <div class="input-group">
                        <textarea id="btn-input" name="content" type="text" class="form-control input-sm" placeholder="お返事をこちらに入力してください！"></textarea>
                        <br>
                        <br>
                      </div>
                      ...
                      <div class="panel-heading">
                          <span class="glyphicon glyphicon-comment"></span> 送信
                      </div>
                    </div>
